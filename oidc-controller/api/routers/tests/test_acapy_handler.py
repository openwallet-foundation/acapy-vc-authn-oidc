...
class TestAcapyHandlerCleanupFunctions:
    ...
    @patch("api.routers.acapy_handler.settings.USE_CONNECTION_BASED_VERIFICATION", True)
    @patch("api.routers.acapy_handler.AcapyClient")
    async def test_cleanup_single_use_connection_successful_cleanup(
        self, mock_acapy_client, mock_auth_session
    ):
        """Test successful cleanup of single-use connection."""
        from api.routers.acapy_handler import _cleanup_single_use_connection

        # Setup single-use auth session
        ...
        mock_client_instance = MagicMock()
        mock_client_instance.delete_presentation_record_and_connection.return_value = (
            True,  # presentation_deleted
            True,  # connection_deleted
            [],  # errors
        )
        mock_acapy_client.return_value = mock_client_instance

        # Execute
        await _cleanup_single_use_connection(mock_auth_session, "test context")
    ...
    @patch("api.routers.acapy_handler.settings.USE_CONNECTION_BASED_VERIFICATION", True)
    @patch("api.routers.acapy_handler.AcapyClient")
    async def test_cleanup_single_use_connection_failed_cleanup_with_errors(
        self, mock_acapy_client, mock_auth_session
    ):
        """Test handling of failed connection cleanup with errors."""
        from api.routers.acapy_handler import _cleanup_single_use_connection
        ...
        # Setup failed cleanup with errors
        mock_client_instance = MagicMock()
        mock_client_instance.delete_presentation_record_and_connection.return_value = (
            False,  # presentation_deleted
            False,  # connection_deleted
            ["Connection not found", "Network timeout"],  # errors
        )
        mock_acapy_client.return_value = mock_client_instance
    ...
    @patch("api.routers.acapy_handler.settings.USE_CONNECTION_BASED_VERIFICATION", True)
    @patch("api.routers.acapy_handler.AcapyClient")
    async def test_cleanup_single_use_connection_exception_handling(
        self, mock_acapy_client, mock_auth_session
    ):
        """Test exception handling during connection cleanup."""
        from api.routers.acapy_handler import _cleanup_single_use_connection
    ...
    async def test_cleanup_single_use_connection_skipped_when_feature_disabled(
        self, mock_auth_session
    ):
        """Test that cleanup is skipped when connection-based verification is disabled."""
        from api.routers.acapy_handler import _cleanup_single_use_connection
        ...
        # Execute - should return early without any client calls
        await _cleanup_single_use_connection(mock_auth_session, "test context")
    ...
    @pytest.mark.asyncio
    @patch("api.routers.acapy_handler.settings.USE_CONNECTION_BASED_VERIFICATION", True)
    async def test_cleanup_single_use_connection_skipped_for_multi_use(
        self, mock_auth_session
    ):
        """Test that cleanup is skipped for multi-use connections."""
        from api.routers.acapy_handler import _cleanup_single_use_connection

        # Setup multi-use auth session
        mock_auth_session.connection_id = "test-connection-id"
        mock_auth_session.multi_use = True
    ...
    @pytest.mark.asyncio
    @patch("api.routers.acapy_handler.settings.USE_CONNECTION_BASED_VERIFICATION", True)
    async def test_cleanup_single_use_connection_skipped_when_no_connection_id(
        self, mock_auth_session
    ):
        """Test that cleanup is skipped when auth session has no connection_id."""
        from api.routers.acapy_handler import _cleanup_single_use_connection

        # Setup auth session without connection_id
        mock_auth_session.connection_id = None
        mock_auth_session.multi_use = False
